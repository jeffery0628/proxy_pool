...
class Mysql_DB():
    ...
    def insert_ip(self, ip, port, scheme, score):
        ...
        if len(res) > 0:
            ...
            self.connect.commit()
            return 0
        insert_sql = """insert into %s (ip,port,scheme,score) values('%s','%s','%s','%s')""" % (
        self.table_name, ip, port, scheme, score)
    ...
    def get_proxies(self, min_score=50, max_score=100):
        query_sql = """select * from %s where score between '%s' and '%s' order by score desc """ % (
        self.table_name, min_score, max_score)
        self.cursor.execute(query_sql)
    ...
    def increase(self, ip):
        update_sql = """update %s a inner join (select * from %s where ip like '%s' ) b on a.id = b.id set a.score = b.score + 5 """ % (
        self.table_name, self.table_name, ip)
    ...
    def decrease(self, ip):
        update_sql = """update %s a inner join (select * from %s where ip like '%s' ) b on a.id = b.id set a.score = b.score - 10 """ % (
        self.table_name, self.table_name, ip)
        self.cursor.execute(update_sql)
        query_sql = """select score from %s where ip like '%s'""" % (self.table_name, ip)
        self.cursor.execute(query_sql)
